...
- **多机型通用化**：机型参数收敛为 `devices/<机型名>/` 可分享模块，未收录机型
  自动提取全套偏移并生成新模块，导入后同内核自动精确命中；素材前自动可行性
  预检（init_boot 误选、MTK 未知族等直接提示不推荐）
...
1. 下载 Release 资产 `ghostlock-x200-root-v1.3.2.zip` 并解压（zip 内含
